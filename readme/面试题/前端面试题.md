...
>
> 通过新建一个数组，把伪数组的元素挪进去就可以变成真数组，比如[...arguments]。
...
箭头函数this指向外层，不能被更改，不能被实例化，没有原型

普通函数this可以更改，可以被实例化，有原型
...
#### return中定义一个对象，有年龄、名字等属性，通过js代码添加一个性别属性（obj.gender）,页面上是否立马显示？如果不能请说出正确解决方案？
